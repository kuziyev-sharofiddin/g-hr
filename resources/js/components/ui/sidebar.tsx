...
function SidebarProvider({
  ...
  const [width, setWidth] = React.useState<string>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("sidebar-width") || SIDEBAR_WIDTH
    }
  ...
  const updateWidth = React.useCallback((newWidth: string) => {
    setWidth(newWidth)
  ...
  const contextValue = React.useMemo<SidebarContext>(
    ...
    [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, width, updateWidth]
  )
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  React.useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const remWidth = newWidth / 16

      if (remWidth >= minWidth && remWidth <= maxWidth) {
        setWidth(`${remWidth}rem`)
      }
    }
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "hover:after:bg-blue-500 hover:after:w-1",
