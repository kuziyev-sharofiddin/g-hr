...
export default function Positions({ positions, filters }: Props) {
    ...
    // Display all positions (active and deleted) together
    const displayedPositions = positions.data;
    ...
    const generatePageNumbers = () => {
        ...
        const totalPages = positions.last_page;
    ...
    const handleSelectAll = () => {
        // Get filterable positions based on current selection type
        const filterablePositions = selectionType === null
            ? displayedPositions.filter(p => !p.deleted_at) // Default to active
            : displayedPositions.filter(p => selectionType === 'deleted' ? p.deleted_at : !p.deleted_at);

        if (selectedRows.size === filterablePositions.length && selectedRows.size > 0) {
            // Deselect all
            ...
            setSelectedRows(new Set(filterablePositions.map((p) => p.id)));
        }
    };
    ...
    const handleBulkDelete = () => {
        ...
        if (confirm(`${selectedRows.size} ta lavozimni o'chirmoqchimisiz?`)) {
            router.post(
                '/documents/positions/bulk-delete',
    ...
    const handleBulkRestore = () => {
        ...
        router.post(
            '/documents/positions/bulk-restore',
            { ids: Array.from(selectedRows) },
            {
                preserveState: true,
                preserveScroll: true,
    ...
    const handleRestore = (position: Position) => {
        router.post(
            `/documents/positions/${position.id}/restore`,
            {},
            {
                preserveState: true,
                preserveScroll: true
    ...
    const handleRowDoubleClick = (position: Position) => {
        setSelectedPosition(position);
        setFormDialogOpen(true);
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                ...
                <div
                    ...
                    <div className="flex items-center justify-between">
                        <h2 className="text-2xl font-semibold">Lavozimlar ro'yhati</h2>
                    </div>
                            ...
                            <TableRow>
                                <TableHead className="w-[50px] text-center">
                                    <input
                                        type="checkbox"
                                        checked={
                                            selectedRows.size === displayedPositions.length &&
                        ...
                        <TableBody>
                            {displayedPositions.length === 0 ? (
                                ...
                                displayedPositions.map((position, index) => (
                                    <TableRow
                                        key={position.id}
                                        className={`cursor-pointer ${position.deleted_at ? 'bg-destructive/5' : ''}`}
                                    ...
                                    >
                                        <TableCell className="text-center">
                                            <input
                                                ...
                                                onChange={() =>
                                                    handleSelectRow(
                                                        position.id,
                                                        position.deleted_at ? 'deleted' : 'active'
                                                    )
                                                }
                                                className="rounded border-gray-600 bg-transparent"
                                            />
                                        ...
                                        <TableCell className="whitespace-nowrap text-center">
                                            <TruncateWithTooltip text={position.name} maxLength={30} />
                                        </TableCell>
                                        <TableCell className="whitespace-nowrap text-center">
                                            {format(new Date(position.created_at), 'dd.MM.yyyy')}
                                        </TableCell>
                                        <TableCell className="text-center">
                    ...
                    {displayedPositions.length > 0 && (
                        <div className="flex items-center justify-between border-t border-sidebar-border/70 px-4 py-3">
                            <div className="text-sm text-muted-foreground">
                                {positions.from}-{positions.to} / {positions.total} ta natija
                            </div>
                                        ...
                                        <PaginationItem>
                                            <PaginationPrevious
                                                onClick={() =>
                                                    ...
                                                    handlePageChange(positions.current_page - 1)
                                                }
                                                className={
                                                    positions.current_page === 1
                                            ...
                                            page === 'ellipsis' ? (
                                                ...
                                                <PaginationItem key={page}>
                                                    <PaginationLink
                                                        ...
                                                        isActive={positions.current_page === page}
